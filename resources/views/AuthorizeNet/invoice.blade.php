            ...
            <div class="card">
                <div class="card-body w-100">
                    <div class="payment-logo text-center pb-3">
                        <img src="{{ asset('custom/AuthorizeNet/images/payment-logo.jpg') }}" alt="payment-logo"
                            loading="lazy">
                    </div>
                    <form class="payment-form" method="post"
                        action="{{ route('store.get.authorizenet.status', ['slug' => $slug, 'status' => 'success']) }}"
                        id="authorizenetForm">
                        @csrf
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-prepend"><span
                                                class="input-group-text">{{ $pre_pay->currency ?? 'USD' }}</span></span>
                                        <input class="form-control" type="number" name="get_amount"
                                            placeholder="Amount" value="{{ $pre_pay->price }}" disabled>
                                        <input type="hidden" name="get_amount" value="{{ $pre_pay->price }}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-12">
                                <div class="form-group mb-3">
                                    <label class="d-block mb-1">{{ __('Card Number') }}<sup aria-hidden="true"
                                            class="text-danger">*</sup></label>
                                    <input class="form-control" pattern="\d{16}" maxlength="16" type="text"
