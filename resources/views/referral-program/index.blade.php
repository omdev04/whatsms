...
@push('script-page')
    ...
    <script>
        ...
        if ($('.is_enable').is(':checked')) {
            ...
            $('.percentage-field').attr("required", true);
            $('.threshold-field').attr("required", true);
            $('.referral-settings').removeClass('disabledCookie');
        } else {
            $('.percentage-field').removeAttr('required');
            $('.threshold-field').removeAttr('required');
                                    ...
                                    <tbody>
                                        @foreach ($transactions as $key => $transaction)
                                            <tr>
                                                <td> {{ ++$key }} </td>
                                                @php
                                                    $owner = \App\Models\User::where('type', 'Owner')
                                ...
                                <table class="table pc-dt-simple" id="payout-request">
                                    ...
                                    <tbody>
                                        @foreach ($payRequests as $key => $transaction)
                                            <tr>
                                                <td> {{ ++$key }} </td>
                                                <td>{{ !empty($transaction->getOwner) ? $transaction->getOwner->name : '-' }}
                                                </td>
                                                <td>{{ $transaction->date }}</td>
                                                <td>{{ $currency . $transaction->req_amount }}</td>
                                                <td>
                                                    <a href="{{ route('amount.request', [$transaction->id, 1]) }}"
                                                        class="btn btn-sm btn-icon  bg-primary text-white me-1" data-bs-toggle="tooltip" title="{{__('Accept')}}">
                                ...
                                <div class="row referral-settings">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            {{ Form::number('percentage', isset($setting) ? $setting->percentage : '', ['class' => 'form-control percentage-field', 'placeholder' => __('Enter Commission Percentage')]) }}
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                {{ Form::number('minimum_threshold_amount', isset($setting) ? $setting->minimum_threshold_amount : '', ['class' => 'form-control threshold-field', 'placeholder' => __('Enter Minimum Payout')]) }}
                                            </div>
                                        </div>
                                    </div>
